...
void main() {
  ...
  test(
    'isolates colliding local repository identifiers and restores the default',
    () async {
      drift.driftRuntimeOptions.dontWarnAboutMultipleDatabases = true;
      addTearDown(() {
        drift.driftRuntimeOptions.dontWarnAboutMultipleDatabases = false;
      ...
      final repositoryA = EdgeProfileRepository('edge-a', factoryA, encryption);
      final repositoryB = EdgeProfileRepository('edge-b', factoryB, encryption);
      final sourceVault = await Vault.fromVaultStore(
        sourceStore,
      ...
      final restoredA =
          (await restored.profileRepositories['edge-a']!.listProfiles()).single;
      final restoredB =
          (await restored.profileRepositories['edge-b']!.listProfiles()).single;
