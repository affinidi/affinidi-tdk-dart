...
void stubFileService(
  ...
  when(
    () => service.deleteFolder(any()),
  ).thenAnswer((_) async => Future<void>.value());
  when(
    () => service.deleteFile(any()),
  ).thenAnswer((_) async => Future<void>.value());
  ...
  when(
    () => service.getNodeInfo(any()),
  ).thenAnswer((_) async => NodeFixtures.mockFileNode(profileId));

  when(
    () => service.downloadFile(nodeId: any(named: 'nodeId')),
...
void stubCredentialService(
  ...
  when(
    () => service.addVerifiableCredentialToProfile(
  ...
  when(
    () => service.deleteClaimedCredential(nodeId: any(named: 'nodeId')),
  ).thenAnswer((_) async => Future<void>.value());
}
