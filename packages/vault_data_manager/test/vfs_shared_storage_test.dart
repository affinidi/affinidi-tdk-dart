...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockDataManagerService.createFile(
    ...
    when(
      () => mockDataManagerService.deleteFolder(any()),
    ).thenAnswer((_) async {});
    when(
      () => mockDataManagerService.getNodeInfo(any()),
    ...
    when(
      () => mockDataManagerService.renameFile(
        nodeId: any(named: 'nodeId'),
        newName: any(named: 'newName'),
      ),
    ).thenAnswer((_) async {});
    when(
      () => mockDataManagerService.renameFolder(
        nodeId: any(named: 'nodeId'),
        newName: any(named: 'newName'),
      ),
    ).thenAnswer((_) async {});
    when(
      () => mockDataManagerService.deleteClaimedCredential(
        nodeId: any(named: 'nodeId'),
      ),
    ).thenAnswer((_) async {});
    when(() => mockDataManagerService.getDigitalCredentials(any())).thenAnswer(
      (_) async => PaginatedList<DigitalCredential>(
        items: <DigitalCredential>[],
        lastEvaluatedItemId: null,
      ),
    );
    when(
      () => mockDataManagerService.addVerifiableCredentialToProfile(
  ...
  group('VfsSharedStorage', () {
    ...
    group('File operations', () {
      test('getFolder returns items from file storage', () async {
        ...
        verify(
          () => mockDataManagerService.getChildNodes(
            nodeId: VfsSharedStorageFixtures.testSharedProfileId,
          ),
        ).called(1);
      });
      ...
      test('deleteFile delegates to file storage', () async {
        ...
        verify(
          () => mockDataManagerService.deleteFile(FileFixtures.testFileId),
        ).called(1);
      });
      ...
      test('getFileContent delegates to file storage', () async {
        ...
        verify(
          () => mockDataManagerService.downloadFile(
    ...
    group('Credential operations', () {
      ...
      test('getCredential delegates to credential storage', () async {
        ...
        when(
          () => mockDataManagerService.getDigitalCredentials(any()),
        ).thenAnswer(
          (_) async => PaginatedList<DigitalCredential>(
        ...
        verify(
          () => mockDataManagerService.getDigitalCredentials(
            VfsSharedStorageFixtures.testSharedProfileId,
          ),
        ).called(1);
      });
